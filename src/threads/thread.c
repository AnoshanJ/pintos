...
/*List of processes in sleep (wait) state (i.e. wait queue).
   Processes are added to this list when they are put to sleep
   and removed when they are woken up. */
...
thread_init (void)
{
  ...
  initial_thread->sleep_endtick = 0; // dummy value
...
/* Called by the timer interrupt handler at each timer tick.
   ...
   The parameter 'tick' is the tick count held by
   the timer device. */
/* Wake up all sleeping threads whose ticks_end has been expired,
   removing from the wait queue and pushing it to the ready queue.
...
thread_awake (int64_t current_tick) {
  ...
  for (ele = list_begin (&wait_list); ele != list_end (&wait_list); ele = list_next (ele))
    {
      struct thread *t = list_entry (ele, struct thread, waitelem);
               ...
               thread_func *function, void *aux)
{
  ...
  /*If the newly created thread has a higher priority than
   ...
   context switching, for thread priority scheduling. */
...
thread_unblock (struct thread *t)
{
  ...
  // t will turn into ready-to-run state by inserting into ready_list
  ...
  // ensures preemption by comparing priorities of current thread and t (to be unblocked),
...
thread_set_priority (int new_priority)
{
  ...
  /* when current thread gets its priority decreased, then yield*/
...
#ifdef USERPROG
  // init process-related information
  t->pcb = NULL;
